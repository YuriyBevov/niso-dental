...
const sliders$2 = document.querySelectorAll(".base-cards-slider");
if (sliders$2.length) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const active = document.querySelector("li:has(li a.selected)");
  if (active) {
...
if (items.length) {
  const fillStars = (index) => {
    items.forEach((item, i) => {
      if (i <= index) {
        !item.parentNode.classList.contains("filled") ? item.parentNode.classList.add("filled") : null;
      } else {
        item.parentNode.classList.contains("filled") ? item.parentNode.classList.remove("filled") : null;
      }
    });
  };
  for (let i = 4; i >= 0; i--) {
...
if (imageComparisonSliders.length) {
  let setSliderstate = function(e, element) {
    ...
    if ("ontouchstart" in window === false) {
      sliderRange.addEventListener(
        "mouseup",
        (evt) => setSliderstate(evt, element)
      );
      sliderRange.addEventListener("mousedown", moveSliderThumb);
    }
    sliderRange.addEventListener(
      "input",
      (evt) => moveSliderRange(evt, element)
    );
    sliderRange.addEventListener(
      "change",
      (evt) => moveSliderRange(evt, element)
  ...
  var setSliderstate2 = setSliderstate, moveSliderThumb2 = moveSliderThumb, moveSliderRange2 = moveSliderRange, init5 = init4;
