...
<!-- video-reviews -->
<?
$linkedVideoReviewsIds = array_filter(
	(array)($arResult['PROPERTIES']['LINKED_VIDEO_REVIEWS']['VALUE'] ?? [])
);
if ($linkedVideoReviewsIds) {
	$GLOBALS['arLinkedVideoReviewsFilter'] = ['ID' => $linkedVideoReviewsIds];
	$APPLICATION->IncludeComponent(
		"bitrix:news.list",
		"video-reviews-new",
		array(
			...
			"DISPLAY_TOP_PAGER" => "N",
			"FIELD_CODE" => array("", "", ""),
			"FILTER_NAME" => "arLinkedVideoReviewsFilter",
			"HIDE_LINK_WHEN_NO_DETAIL" => "N",
			"IBLOCK_ID" => "43",
			"IBLOCK_TYPE" => "site_content",
			"INCLUDE_IBLOCK_INTO_CHAIN" => "N",
